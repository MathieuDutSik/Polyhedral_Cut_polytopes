...
EXT:=ReadAsFunction("Functions/TheEXT35")();
DistEXT:=FuncDistMat(EXT);
DM:=FuncDistMat(EXT);
GRP:=AutomorphismGroupEdgeColoredGraph(DM);
O:=Orbits(GRP, [1..Length(EXT)], OnPoints);

GetSpecialAffineBasis:=function(DiffVertex)
  local EXTred, AffBasis, IsOK, eVert, B;
  EXTred:=Difference(EXT, [EXT[DiffVertex]]);
  while(true)
  do
...
Print("Before generation of the conjectured list of facets\n");
ComputeHypermetricInformations();
Print("After generation of the conjectured list of facets\n");
...
do
  SelVert:=eO[1];
  VertSubset:=Difference([1..Length(EXT)], [SelVert]);
  AffBasis:=GetSpecialAffineBasis(SelVert);
  eOrb:=List(AffBasis, x->Position(EXT, x));
  TheResult:=FindAdjacentDelaunay(eOrb, VertSubset);
