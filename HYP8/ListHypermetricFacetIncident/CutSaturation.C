...
void FirstCutSet(int&n, vector<int> & TheCut)
{
  int iCol;
...
void NextCutSet(int&n, vector<int> & TheCut, int & result)
{
  int MatchedCol, iCol;
  MatchedCol=-2;
  for (iCol=1; iCol<=n-1; iCol++)
    if (TheCut[iCol-1] == 0 && MatchedCol == -2)
      MatchedCol=iCol;
  if (MatchedCol == -2)
    {
      result=0;
    ...
    {
      for (iCol=1; iCol<MatchedCol; iCol++)
	TheCut[iCol-1]=0;
      TheCut[MatchedCol-1]=1;
    }
  result=1;
}
...
void PrintCutSystem(int& n, 
		    vector<vector<int> > & ListCuts, 
		    FILE *OUTPUT)
{
  int IsFirstPrint, iCut, iCol, eVal;
  set<vector<int> >::iterator iter;
  vector<int> eCut;

  IsFirstPrint=1;
  fprintf(OUTPUT, "return [");
  for (iCut=1; iCut<=ListCuts.size(); iCut++)
    {
      eCut=ListCuts[iCut-1];
      if (IsFirstPrint == 0)
	{
	  fprintf(OUTPUT, ",\n");
	}
...
void DetermineIncidentCuts(int &n, 
			   vector<set<vector<int> > > & ListIncident, 
			   vector<vector<int> > & ListCutSaturation)
{
  vector<int> TheCut;
  int IsOK, iOrb, iCol, result;
  set<vector<int> >::iterator iter;
  vector<int> eHyp;
  int TheSum;
  FirstCutSet(n, TheCut);
  while(1)
    {
      IsOK=1;
      for (iOrb=1; iOrb<=ListIncident.size(); iOrb++)
	{
	  iter=ListIncident[iOrb-1].begin();
	  while(iter != ListIncident[iOrb-1].end())
	    {
      ...
      if (IsOK == 1)
	{
	  ListCutSaturation.push_back(TheCut);
	}
      NextCutSet(n, TheCut, result);
      if (result == 0)
	{
	  break;
	}
...
int main(int argc, char *argv[])
{
  ...
  //
  DetermineIncidentCuts(n, ListIncident, ListCutSaturation);
  //
  OUTPUT=fopen(argv[4], "w");
  PrintCutSystem(n, ListCutSaturation, OUTPUT);
  fclose(OUTPUT);
